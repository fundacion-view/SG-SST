...
function dselect(el, option = {}) {
  ...
  const searchInput = search
    ? `<input onkeydown="return event.key !== 'Enter'" onkeyup="dselectSearch(event, this, '${classElement}', '${classToggler}', ${creatable})" type="text" class="form-control bg-primary text-white" placeholder="Búsqueda" autofocus>`
    : "";
  ...
  function selectedTag(options, multiple) {
    if (multiple) {
      const selectedOptions = Array.from(options).filter(
        (option) => option.selected && !isPlaceholder(option)
      );
      const placeholderOption = Array.from(options).filter((option) =>
  ...
  function itemTags(options) {
    ...
    for (const option of options) {
      if (option.tagName === "OPTGROUP") {
        items.push(
          `<h6 class="dropdown-header">${option.getAttribute("label")}</h6>`
        );
      } else {
    ...
    <div class="dropdown ${classElement} ${additionalClass}">
      ...
      <div class="dropdown-menu">
        <div class="d-flex flex-column">
          ...
          <div class="${classNoResults} d-none">Sin resultados</div>
        </div>
      </div>
      ${clearBtn}
    </div>
